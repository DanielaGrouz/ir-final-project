...
def get_bm25_scores(query: str):
    ...
    fields = [
        (body_inverted_index, BODY_STATS, 0.1, simple_tokenize, DL_BY_FIELD["body"]),
        (title_inverted_index_smart, TITLE_STATS, 0.6, tokenize_smart, DL_BY_FIELD["title"]),
...
def rank_candidates(query, top_candidates_scores):
    ...
    for i, cid in enumerate(candidate_ids):
        ...
        final_score = (bm25_val * 0.7) + (sem_val * 0.2) + (pr_val * 0.05) + (pv_val * 0.05)
...
def search():
    ...
    if query_tokens:
        ...
        if all_bm25_scores:
            top_150 = sorted(all_bm25_scores.items(), key=lambda x: x[1], reverse=True)[:200]
