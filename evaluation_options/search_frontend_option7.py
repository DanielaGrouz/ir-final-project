...
def get_bm25_scores(query: str):
    ...
    fields = [
        (body_inverted_index, BODY_STATS, 0.15, simple_tokenize, DL_BY_FIELD["body"]),
        (title_inverted_index_smart, TITLE_STATS, 0.7, tokenize_smart, DL_BY_FIELD["title"]),
        (anchor_inverted_index_smart, ANCHOR_STATS, 0.15, tokenize_anchor, DL_BY_FIELD["anchor"])
    ]
...
def rank_candidates(query, top_candidates_scores):
    ...
    for i, cid in enumerate(candidate_ids):
        ...
        final_score = (bm25_val * 0.4) + (sem_val * 0.3) + (pr_val * 0.2) + (pv_val * 0.1)
