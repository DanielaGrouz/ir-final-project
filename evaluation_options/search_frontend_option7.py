...
def get_bm25_scores(query: str):
    ...
    fields = [
        (body_inverted_index, BODY_STATS, 0.15, simple_tokenize, DL_BY_FIELD["body"]),  # Low noise
        (title_inverted_index_smart, TITLE_STATS, 0.7, tokenize_smart, DL_BY_FIELD["title"]), # Max Signal
        (anchor_inverted_index_smart, ANCHOR_STATS, 0.15, tokenize_anchor, DL_BY_FIELD["anchor"]) # High Signal
    ]
...
def rank_candidates(query, top_candidates_scores):
    ...
    for i, cid in enumerate(candidate_ids):
        final_score = (
            (bm25_norm[i] * 0.4) +  (sem_norm[i]  * 0.3) +   (pv_norm[i]   * 0.1) +   (pr_norm[i]   * 0.2)    
        )
...
def search():
    ...
    if query_tokens:
        ...
        if all_bm25_scores:
            # use a heap to efficiently find top K scores (O(N log K))
            top_candidates = heapq.nlargest(150, all_bm25_scores.items(), key=lambda x: x[1])
